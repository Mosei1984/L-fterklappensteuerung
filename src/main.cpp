...
class FlashSettingsStorage final : public SettingsStoragePort {
 public:
  bool read(std::uint8_t* const data, const std::size_t size) override {
    if (!ensureReady() || (size > sectorSize_)) {
      return false;
    }

    return flash_.read(data, storageAddress_, static_cast<std::uint32_t>(size)) ==
           0;
  }

  bool write(const std::uint8_t* const data, const std::size_t size) override {
    if (!ensureReady() || (size > sectorSize_) ||
        (sectorSize_ > scratch_.size())) {
      return false;
    }
    ...
    if (flash_.erase(storageAddress_, sectorSize_) != 0) {
      return false;
    }

    return flash_.program(scratch_.data(), storageAddress_, sectorSize_) == 0;
  }
...
#if LUEFTERKLAPPE_USE_TMC2209_UART
  const bool stallGuardActive =
      (stepper.speed() != 0.0F) && tmcDriver.pollStallGuard();
#else
  constexpr bool stallGuardActive = false;
#endif

  const DigitalInputs inputs{readSwitch(kMinSwitchPin), readSwitch(kMaxSwitchPin),
                             stallGuardActive};
