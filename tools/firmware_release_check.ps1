...
function Assert-ModbusDiagnostics {
    ...
    $readAll = New-ModbusFrame -Payload ([byte[]] @($DeviceId, 0x03, 0x00, 0x00, 0x00, 0x25))
    $readDiagnostics = New-ModbusFrame -Payload ([byte[]] @($DeviceId, 0x03, 0x00, 0x11, 0x00, 0x06))
    $readPastEnd = New-ModbusFrame -Payload ([byte[]] @($DeviceId, 0x03, 0x00, 0x24, 0x00, 0x02))
    ...
    Add-ReportBlock -Title "Modbus read 0..36" -Lines @([BitConverter]::ToString($readAllResponse))
    if (($readAllResponse.Length -lt 5) -or (-not (Test-ModbusCrc -Frame $readAllResponse))) {
        throw 'Modbus read 0..36 returned no valid CRC frame'
    }
    ...
    Add-ReportBlock -Title "Modbus read past 36" -Lines @([BitConverter]::ToString($readPastEndResponse))
    if (($readPastEndResponse.Length -lt 5) -or (-not (Test-ModbusCrc -Frame $readPastEndResponse)) -or
        ...
        throw 'Modbus read past 36 did not return illegal-address exception'
    }
