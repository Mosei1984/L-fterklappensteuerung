...
public sealed class OpenHabExportAdapter : IHomeAutomationExportAdapter
{
    ...
    public ExportArtifact Export(ControllerProfile profile, byte deviceId)
    {
        ...
        builder.AppendLine(CultureInfo.InvariantCulture, $"Number FanFlap{deviceId}_HomeMinSwitch \"Home Min Switch\" {{ channel=\"modbus:data:fanflap{deviceId}:home_min_switch\" }}");
        builder.AppendLine(CultureInfo.InvariantCulture, $"Switch FanFlap{deviceId}_StepperInverted \"Stepper Direction Inverted\" {{ channel=\"modbus:data:fanflap{deviceId}:stepper_direction_inverted\" }}");
